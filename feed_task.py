...
bot = commands.Bot(command_prefix="!", help_command=None, intents=intents, self_bot=True)
...
@bot.event
async def on_ready():
    print(f"✅ Bot Feed Waifu Đa Kênh Đã Sẵn Sàng!", flush=True)
    if not auto_feed_loop.is_running():
        auto_feed_loop.start()

try:
    token = os.environ.get("TOKEN")
    if not token:
        print("❌ Không tìm thấy biến môi trường 'TOKEN'. Vui lòng kiểm tra lại environment.", flush=True)
    else:
