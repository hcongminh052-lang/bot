...
class FeedTask(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.auto_feed_loop.start()

    def cog_unload(self):
        self.auto_feed_loop.cancel()
    ...
    def google_search_answer(self, question_text):
    ...
    async def auto_feed_loop(self):
        ...
        if channel:
            try:
                if self.auto_feed_loop.current_loop > 0:
    ...
    async def on_message(self, message):
        ...
        if message.author.id == BOT_GAME_ID and message.embeds:
            ...
            if "CÂU HỎI FEED" in title or "Reply trực tiếp" in description:
                ...
                answer = await loop.run_in_executor(None, self.google_search_answer, description)
